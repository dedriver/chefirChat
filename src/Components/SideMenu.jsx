...
export default function SideMenu() {
    ...
    return (
        <div>
            <div
                className={`${
                    activeMenu ? "animate-slideInLeft left-0" : "left-[-100%]"
                } bg-fone absolute h-screen w-52 p-4 flex flex-col justify-between transition-all duration-300`}
                style={{
                    top: 0,
                }}
            >
                ...
                <div>
                    <button className="w-full bg-red-700 text-black py-2 rounded-lg hover:bg-yellow-500 focus:ring focus:ring-yellow-300">
                        Log Out
                    </button>
                </div>
            </div>
            {/* Для маленького екрану або коли меню сховано */}
            {!activeMenu && (
                <div className="bg-fone h-screen w-max p-4 flex flex-col justify-between">
                    <div>
                        ...
                        >
                            <CiMenuBurger className="transform transition-transform duration-300 rotate-90" />
                        </button>
